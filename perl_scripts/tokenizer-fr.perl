...
my $mydir = "/group/project/statmt2/josh/bin/";
...
sub load_prefixes {
	...
	if ($language eq "fr") {
		#In french
		my @FRENCH_PREFIXES = ("alb","av","avr","cell","cf","ch","chap","corp","div","d","dr","e","ed","env","ét","etc","ex","ext","h","hab","i","id","inc","incl","inf","j","janv","juill","ltd","m","mm","mawsc","max","min","n","oct","nov","ont","p","pp","qq","réf","s","sect","sem","sept","sing","sask","sup","tél","téléc","trim","univ","v","vol","vols");
		#Add all numbers 1-99
		for (my $i=1;$i<100;$i++) {
			push(@FRENCH_PREFIXES,"".$i);	
		}
		foreach my $prefix (@FRENCH_PREFIXES) {
			$PREFIX_REF->{$prefix} = 1;
		}
	}
} 	
